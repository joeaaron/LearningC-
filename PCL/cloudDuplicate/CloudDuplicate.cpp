...
const float RADIUS = 0.02;				// 若两点之间的距离为0.000001则认为是重合点
...
int main()
{
	...
	if (pcl::io::loadPCDFile<pcl::PointXYZ>("test.pcd", *cloud) == -1)			// bunnyDuplicate.pcd
	{
	...
	std::cout << "kdtree去除重叠点平均用时： " << dAverageTime << "ms" << std::endl;
	std::cout << "kdtree去除重叠点最大用时： " << dMaxTime << " ms" << std::endl;
