...
int main()
{
	...
	time.tic();
	std::string filename1 = "pig_view1.pcd";//点云文件名
	...
	*cloud_icp_in = *cloud_in_tgt;
	...
	std::string filename2 = "pig_view2.pcd";//点云文件名
	if (pcl::io::loadPCDFile<pcl::PointXYZ>(filename2, *cloud_toberegistration) == -1)//*打开点云文件	
	{
		PCL_ERROR("Couldn't read file test_pcd.pcd\n");//输出错误原因：读取文件失败
		return(-1);//程序结束
	}
